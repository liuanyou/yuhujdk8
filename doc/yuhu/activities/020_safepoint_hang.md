...
### 方案 2 的终极方案：参数传递简化设计（021 文档）

**注意**：方案 2 的显式保存 LR 和 FP 是一个**临时修复**，解决了症状，但没有解决根本原因。

**根本问题**：当前设计中的参数传递流程过于复杂，导致栈帧布局难以符合 HotSpot 标准。

**终极解决方案**：参见 `021_parameter_passing_simplification.md`

**核心思想**：
- 让 LLVM 函数的参数直接对应 Java 方法的参数
- Method* 和 Thread* 通过寄存器读取（x12, x28），不通过参数传递
- 简化适配器代码，移除临时栈区域和参数打包
...
**实施建议**：
- 先实施方案 2（临时修复）：快速解决 safepoint 卡死问题
- 再实施 021 的简化设计（根本重构）：从根本上解决问题，简化代码
