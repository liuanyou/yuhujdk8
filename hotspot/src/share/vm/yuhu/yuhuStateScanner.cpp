...
void YuhuStateScanner::scan(YuhuState* state) {
  start_frame();
...
#ifndef PRODUCT
void YuhuStateScanner::stack_integrity_checks(YuhuState* state) {
...
void YuhuStateScanner::locals_integrity_checks(YuhuState* state) {
  for (int i = 0; i < max_locals(); i++) {
    if (state->local(i)) {
