...
void YuhuNativeWrapper::initialize(const char *name) {
  // Create the function
  ...
  OopMap *oopmap = new OopMap(
    YuhuStack::oopmap_slot_munge(stack()->oopmap_frame_size()),
  ...
  param_values.push_back(
    builder()->CreateAddressOfStructEntry(
      thread,  // thread is llvm::Argument*, not a function call
      JavaThread::jni_environment_offset(),
      YuhuType::jniEnv_type(),
      "jni_environment"));
  ...
  for (int i = 0; i < arg_size(); i++) {
    ...
    case T_ARRAY:
      ...
      oopmap->set_oop(YuhuStack::slot2reg(slot_offset));
    ...
    default:
      llvm::Type *param_type = YuhuType::to_stackType(arg_type(i));  // Use llvm::Type to avoid conflict

      param_types.push_back(param_type);
      param_values.push_back(
        builder()->CreateLoad(param_type, stack()->slot_addr(adjusted_offset, param_type)));
    }
  }
  ...
  llvm::Type* return_type;  // Use llvm::Type to avoid conflict
  if (result_type == T_VOID)
    return_type = YuhuType::void_type();
  else if (is_returning_oop())
    return_type = box_type;
  else
    return_type = YuhuType::to_arrayType(result_type);
  llvm::FunctionType* func_type = llvm::FunctionType::get(return_type, param_types, false);
  Value* native_function = builder()->CreateIntToPtr(
     LLVMValue::intptr_constant((intptr_t) target()->native_function()),
     PointerType::getUnqual(func_type));
#if LLVM_VERSION_MAJOR >= 20
  Value *result = builder()->CreateCall(
    func_type, native_function, llvm::ArrayRef<Value*>(param_values));
#else
  Value *result = builder()->CreateCall(
  ...
  // check_special_condition_for_native_trans signature: "T" -> "v" (Thread* -> void)
#if LLVM_VERSION_MAJOR >= 20
  llvm::FunctionType* check_func_type = YuhuBuilder::make_ftype("T", "v");
  std::vector<Value*> args;
  args.push_back(thread);
  builder()->CreateCall(
    check_func_type, builder()->check_special_condition_for_native_trans(), args);
#else
  builder()->CreateCall(
  ...
  if (is_returning_oop()) {
    ...
    builder()->SetInsertPoint(not_null);
    // result is a pointer to the boxed value, need to load it
    // The type depends on the return type, but for now assume it's the same as return_type
    Value *unboxed_result = builder()->CreateLoad(return_type, result);
    builder()->CreateBr(merge);
