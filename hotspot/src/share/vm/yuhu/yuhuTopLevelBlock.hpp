...
#ifndef SHARE_VM_YUHU_YUHUTOPLEVELBLOCK_HPP
#define SHARE_VM_YUHU_YUHUTOPLEVELBLOCK_HPP
...
class YuhuTopLevelBlock : public YuhuBlock {
 ...
 public:
  ...
  YuhuTopLevelBlock* successor(int index) const {
    return function()->block(ciblock()->successors()->at(index)->pre_order());
  }
  YuhuTopLevelBlock* bci_successor(int bci) const;

  // Exceptions
 private:
  GrowableArray<ciExceptionHandler*>* _exc_handlers;
  GrowableArray<YuhuTopLevelBlock*>* _exceptions;
 ...
 private:
  ...
  YuhuTopLevelBlock* exception(int index) const {
 ...
 private:
  void enter(YuhuTopLevelBlock* predecessor, bool is_exception);
 ...
 private:
  ...
  int set_num_monitors(int num_monitors) {
    ...
    return num_monitors;  // Return value to satisfy return type
  }
 ...
 private:
  ...
  // Zero checks
 private:
  void do_zero_check(YuhuValue* value);
  void zero_check_value(YuhuValue* value, llvm::BasicBlock* continue_block);

 public:
  void do_deferred_zero_check(YuhuValue*       value,
                              int               bci,
 ...
 public:
  ...
  // VM calls
 private:
  // Overload for void return type (most common case)
  llvm::CallInst* call_vm(llvm::Value*  callee,
                          llvm::Value** args_start,
                          llvm::Value** args_end,
                          int           exception_action) {
    return call_vm(callee, args_start, args_end, exception_action, YuhuType::void_type());
  }
  
  // Overload with explicit return type (for calls that return values)
  llvm::CallInst* call_vm(llvm::Value*  callee,
                          llvm::Value** args_start,
                          llvm::Value** args_end,
                          int           exception_action,
    ...
    llvm::ArrayRef<llvm::Value*> args_array(args_start, args_end);
    std::vector<llvm::Type*> param_types;
    for (llvm::Value** arg = args_start; arg != args_end; ++arg) {
 ...
 public:
  ...
  // Bounds checks
 private:
  void check_bounds(YuhuValue* array, YuhuValue* index);
 ...
 private:
  ...
  bool can_reach(YuhuTopLevelBlock* other);
  bool can_reach_helper(YuhuTopLevelBlock* other);
 ...
 private:
  ...
  void do_if(llvm::ICmpInst::Predicate p, YuhuValue* b, YuhuValue* a);
 ...
 private:
  ...
  llvm::Value* get_dynamic_resolution_callee(YuhuValue* receiver, ciMethod* call_method, address* out_stub_addr, GrowableArray<BasicType>* reg_basic_types, GrowableArray<BasicType>* stk_basic_types);
...
#endif // SHARE_VM_YUHU_YUHUTOPLEVELBLOCK_HPP
