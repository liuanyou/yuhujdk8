...
#ifndef SHARE_VM_YUHU_YUHUNATIVEWRAPPER_HPP
#define SHARE_VM_YUHU_YUHUNATIVEWRAPPER_HPP
...
class YuhuNativeWrapper : public YuhuCompileInvariants {
  friend class YuhuStackWithNativeFrame;

 public:
  static YuhuNativeWrapper* build(YuhuBuilder* builder,
                                   methodHandle  target,
                                   const char*   name,
                                   BasicType*    arg_types,
                                   BasicType     return_type) {
    return new YuhuNativeWrapper(builder,
                                  target,
                                  name,
                                  arg_types,
                                  return_type);
 ...
 private:
  YuhuNativeWrapper(YuhuBuilder* builder,
                     methodHandle  target,
                     const char*   name,
                     BasicType*    arg_types,
                     BasicType     return_type)
    : YuhuCompileInvariants(NULL, builder),
      _target(target),
 ...
 private:
  llvm::BasicBlock* CreateBlock(const char* name = "") const {
    return llvm::BasicBlock::Create(YuhuContext::current(), name, function());
  }
  llvm::Value* thread_state_address() const {
    return builder()->CreateAddressOfStructEntry(
      thread(), JavaThread::thread_state_offset(),
      llvm::PointerType::getUnqual(YuhuType::jint_type()),
    ...
    builder()->CreateStore(
      ...
      builder()->CreateIntToPtr(
        builder()->CreateAdd(
          ...
          builder()->CreateAnd(
            builder()->CreateLShr(
              builder()->CreatePtrToInt(thread(), YuhuType::intptr_type()),
...
#endif // SHARE_VM_YUHU_YUHUNATIVEWRAPPER_HPP
