...
#ifndef SHARE_VM_YUHU_YUHUBLOCK_HPP
#define SHARE_VM_YUHU_YUHUBLOCK_HPP
...
class YuhuBlock : public YuhuTargetInvariants {
 protected:
  YuhuBlock(const YuhuTargetInvariants* parent)
    : YuhuTargetInvariants(parent),
      _iter(target()),
      _current_state(NULL) {}

  YuhuBlock(const YuhuCompileInvariants* parent, ciMethod* target)
    : YuhuTargetInvariants(parent, target),
      _iter(target),
      _current_state(NULL) {}

 private:
  ciBytecodeStream _iter;
  YuhuState*      _current_state;
 ...
 public:
  ...
  // Entry state
 protected:
  virtual YuhuState* entry_state();

  // Current state
 private:
  YuhuState* initial_current_state();

 public:
  YuhuState* current_state() {
 ...
 protected:
  void set_current_state(YuhuState* current_state) {
    _current_state = current_state;
  }

  // Local variables
 protected:
  YuhuValue* local(int index) {
    YuhuValue *value = current_state()->local(index);
 ...
 protected:
  void xpush(YuhuValue* value) {
    current_state()->push(value);
  }
  YuhuValue* xpop() {
    return current_state()->pop();
  }
  YuhuValue* xstack(int slot) {
    YuhuValue *value = current_state()->stack(slot);
    assert(value != NULL, "shouldn't be");
    assert(value->is_one_word() ||
           (slot > 0 &&
 ...
 protected:
  ...
  YuhuValue* pop() {
    ...
    if (size == 2)
      xpop();
    YuhuValue *value = xpop();
    assert(value && value->size() == size, "should be");
    return value;
  }
  YuhuValue* pop_result(BasicType type) {
    YuhuValue *result = pop();
 ...
 protected:
  ...
  // Helpers
 protected:
  virtual void do_zero_check(YuhuValue* value);

  // Zero checking
 protected:
  void check_null(YuhuValue* object) {
    zero_check(object);
  }
  void check_divide_by_zero(YuhuValue* value) {
    zero_check(value);
  }
 private:
  void zero_check(YuhuValue* value) {
 ...
 protected:
  ...
  // if*
 protected:
  virtual void do_if(llvm::ICmpInst::Predicate p, YuhuValue* b, YuhuValue* a);
...
#endif // SHARE_VM_YUHU_YUHUBLOCK_HPP
