...
address YuhuMacroAssembler::write_insts_orptr(YuhuAddress adr, YuhuRegisterOrConstant src) {
    write_inst_ldr(x9, adr);
    if (src.is_register())
        write_inst_regs("orr %s, %s, %s", x9, x9, src.as_register());
    else
        write_inst("orr %s, %s, #%d", x9, x9, src.as_constant());
    write_inst_str(x9, adr);
    return current_pc();
}
...
//    }
    ...
    if (table == YuhuInterpreter::dispatch_table(state)) {
        ...
        write_insts_mov_imm64(x9, (uint64_t)table);
        write_inst("ldr x9, [x9, w8, uxtw #3]");
    }
    write_inst("br x9");
    return current_pc();
}
...
address YuhuMacroAssembler::write_insts_call_VM_base(YuhuRegister oop_result, YuhuRegister java_thread, YuhuRegister last_java_sp, address entry_point, int number_of_arguments, bool check_exceptions) {
...
#ifdef ASSERT
    {
        ...
        write_inst_cbz(x8, L);
        write_insts_stop("YuhuMacroAssembler::write_insts_call_VM_base:"
             " last_sp != NULL");
        pin_label(L);
    }
#endif /* ASSERT */
...
address YuhuMacroAssembler::write_insts_final_call_VM_base(YuhuRegister oop_result, YuhuRegister java_thread, YuhuRegister last_java_sp,
    ...
    // TODO - skip it for now, it is for JVMTI/debugging
...
int YuhuMacroAssembler::write_insts_biased_locking_enter(YuhuRegister lock_reg,
    ...
    do_assert_different_registers(lock_reg, obj_reg, swap_reg);

    if (PrintBiasedLockingStatistics && counters == NULL)
        counters = BiasedLocking::counters();

    do_assert_different_registers(lock_reg, obj_reg, swap_reg, tmp_reg, x8, x9, noreg);
...
address YuhuMacroAssembler::write_insts_g1_write_barrier_pre(YuhuRegister obj,
    ...
    do_assert_different_registers(obj, pre_val, tmp, x8);
...
address YuhuMacroAssembler::write_insts_g1_write_barrier_post(YuhuRegister store_addr,
    ...
    assert(thread == x28, "must be");
#endif // _LP64
    do_assert_different_registers(store_addr, new_val, thread, tmp, tmp2,
                               x8);
...
address YuhuMacroAssembler::write_insts_get_cache_and_index_at_bcp(YuhuRegister cache,
                                                           ...
                                                           int bcp_offset,
                                                           size_t index_size) {
    do_assert_different_registers(cache, index);
    do_assert_different_registers(cache, x26);
...
address YuhuMacroAssembler::write_insts_gen_subtype_check(YuhuRegister Rsub_klass,
    ...
    write_insts_profile_typecheck(x2, Rsub_klass, x5); // blows r2, reloads r5

    // Do the check.
    write_insts_check_klass_subtype(Rsub_klass, x0, x2, ok_is_subtype); // blows r2

    // Profile the failure of the check.
    write_insts_profile_typecheck_failed(x2); // blows r2
...
address YuhuMacroAssembler::write_insts_check_klass_subtype_fast_path(YuhuRegister sub_klass,
    ...
    do_assert_different_registers(sub_klass, super_klass, temp_reg);
    bool must_load_sco = (super_check_offset.constant_or_zero() == -1);
    if (super_check_offset.is_register()) {
        do_assert_different_registers(sub_klass, super_klass,
...
address YuhuMacroAssembler::write_insts_check_klass_subtype_slow_path(YuhuRegister sub_klass,
    ...
    do_assert_different_registers(sub_klass, super_klass, temp_reg);
    if (temp2_reg != noreg)
        do_assert_different_registers(sub_klass, super_klass, temp_reg, temp2_reg, x8);
...
address YuhuMacroAssembler::write_insts_encode_heap_oop(YuhuRegister d, YuhuRegister s) {
    // TODO - skip it for now, it is for debugging
//#ifdef ASSERT
...
address YuhuMacroAssembler::write_insts_load_resolved_reference_at_index(
    ...
    do_assert_different_registers(result, index);
...
YuhuAddress YuhuMacroAssembler::form_address(YuhuRegister Rd, YuhuRegister base, int64_t byte_offset, int shift) {
    // TODO - skip it for now, it is for performance optimization
...
address YuhuMacroAssembler::write_insts_lookup_interface_method(YuhuRegister recv_klass,
                                                                ...
                                                                bool return_method) {
    do_assert_different_registers(recv_klass, intf_klass, scan_temp);
    do_assert_different_registers(method_result, intf_klass, scan_temp);
    ...
    if (HeapWordsPerLong > 1) {
        ...
        // TODO - skip it for now, it is always false on aarch64, it is only for 32-bit platforms
...
address YuhuMacroAssembler::write_insts_tlab_allocate(YuhuRegister obj,
                                                      ...
                                                      YuhuRegister t2,
                                                      YuhuLabel& slow_case) {
    do_assert_different_registers(obj, t2);
    do_assert_different_registers(obj, var_size_in_bytes);
    YuhuRegister end = t2;
...
address YuhuMacroAssembler::write_insts_eden_allocate(YuhuRegister obj,
                                                      ...
                                                      YuhuRegister t1,
                                                      YuhuLabel& slow_case) {
    do_assert_different_registers(obj, var_size_in_bytes, t1);
...
address YuhuMacroAssembler::write_insts_set_mdp_data_at(YuhuRegister mdp_in, int constant, YuhuRegister value) {
    assert(ProfileInterpreter, "must be profiling interpreter");
    YuhuAddress data(mdp_in, constant);
    write_inst_str(value, data);
    return current_pc();
}


address YuhuMacroAssembler::write_insts_increment_mdp_data_at(YuhuRegister mdp_in, int constant, bool decrement) {
    write_insts_increment_mdp_data_at(mdp_in, noreg, constant, decrement);
    return current_pc();
}

address YuhuMacroAssembler::write_insts_increment_mdp_data_at(YuhuRegister mdp_in, YuhuRegister reg, int constant, bool decrement) {
    assert(ProfileInterpreter, "must be profiling interpreter");
    // %%% this does 64bit counters at best it is wasting space
    // at worst it is a rare bug when counters overflow

    do_assert_different_registers(x9, x8, mdp_in, reg);

    YuhuAddress addr1(mdp_in, constant);
    YuhuAddress addr2(x9, reg, YuhuAddress::lsl(0));
    YuhuAddress &addr = addr1;
    if (reg != noreg) {
        write_insts_lea(x9, addr1);
        addr = addr2;
    }
    ...
    if (decrement) {
        ...
        // addptr(data, (int32_t) DataLayout::counter_increment);
        // so we do this
        write_inst_ldr(x8, addr);
        write_inst("subs %s, %s, #%d", x8, x8, (unsigned)DataLayout::counter_increment);
        YuhuLabel L;
        write_inst_b(lo, L);       // skip store if counter overflow
        write_inst_str(x8, addr);
        pin_label(L);
    } else {
        ...
        // so we do this
        write_inst_ldr(x8, addr);
        write_inst("adds %s, %s, #%d", x8, x8, DataLayout::counter_increment);
        YuhuLabel L;
        write_inst_b(cs, L);       // skip store if counter overflow
        write_inst_str(x8, addr);
        pin_label(L);
    }
    return current_pc();
...
address YuhuMacroAssembler::write_insts_update_mdp_by_constant(YuhuRegister mdp_in, int constant) {
    assert(ProfileInterpreter, "must be profiling interpreter");
    write_inst("add %s, %s, #%d", mdp_in, mdp_in, (unsigned)constant);
    write_inst_str(mdp_in, YuhuAddress(x29, frame::interpreter_frame_mdx_offset * wordSize));
    return current_pc();
}

// Jump if ((*counter_addr += increment) & mask) satisfies the condition.
address YuhuMacroAssembler::write_insts_increment_mask_and_jump(YuhuAddress counter_addr,
                                                                int increment, int mask,
                                                                YuhuRegister scratch, YuhuRegister scratch2,
                                                                bool preloaded,
                                                                YuhuCond cond, YuhuLabel* where) {
    if (!preloaded) {
        write_inst_ldr(w_reg(scratch), counter_addr);
    }
    write_inst("add %s, %s, #%d", scratch, scratch, increment);
    write_inst_str(w_reg(scratch), counter_addr);
    if (operand_valid_for_logical_immediate(/*is32*/true, mask)) {
        write_inst("ands %s, %s, #%d", w_reg(scratch), w_reg(scratch), mask);
    } else {
        write_insts_mov_imm32(w_reg(scratch), (unsigned)mask);
        write_inst("ands %s, %s, %s", w_reg(scratch), w_reg(scratch), w_reg(scratch2));
    }
    write_inst_b(cond, *where);
    return current_pc();
}

address YuhuMacroAssembler::write_insts_get_method_counters(YuhuRegister method,
                                                            YuhuRegister mcs, YuhuLabel& skip) {
    YuhuLabel has_counters;
    write_inst_ldr(mcs, YuhuAddress(method, Method::method_counters_offset()));
    write_inst_cbnz(mcs, has_counters);
    write_insts_final_call_VM(noreg, CAST_FROM_FN_PTR(address,
                                    InterpreterRuntime::build_method_counters), method);
    write_inst_ldr(mcs, YuhuAddress(method, Method::method_counters_offset()));
    write_inst_cbz(mcs, skip); // No MethodCounters allocated, OutOfMemory
    pin_label(has_counters);
    return current_pc();
}

address YuhuMacroAssembler::write_insts_test_method_data_pointer(YuhuRegister mdp,
                                                                YuhuLabel& zero_continue) {
    assert(ProfileInterpreter, "must be profiling interpreter");
    write_inst_ldr(mdp, YuhuAddress(x29, frame::interpreter_frame_mdx_offset * wordSize));
    write_inst_cbz(mdp, zero_continue);
    return current_pc();
}

address YuhuMacroAssembler::write_insts_test_mdp_data_at(YuhuRegister mdp_in, int offset, YuhuRegister value, YuhuRegister test_value_out, YuhuLabel& not_equal_continue) {
    assert(ProfileInterpreter, "must be profiling interpreter");
    if (test_value_out == noreg) {
        write_inst_ldr(x8, YuhuAddress(mdp_in, offset));
        write_inst_regs("cmp %s, %s", value, x8);
    } else {
        // Put the test value into a register, so caller can use it:
        write_inst_ldr(test_value_out, YuhuAddress(mdp_in, offset));
        write_inst_regs("cmp %s, %s", value, test_value_out);
    }
    write_inst_b(ne, not_equal_continue);
    return current_pc();
}

// Set the method data pointer for the current bcp.
address YuhuMacroAssembler::write_insts_set_method_data_pointer_for_bcp() {
    assert(ProfileInterpreter, "must be profiling interpreter");
    YuhuLabel set_mdp;
    write_inst_stp(x0, x1, YuhuAddress(YuhuPre(sp, -2 * wordSize)));

    // Test MDO to avoid the call if it is NULL.
    write_inst_ldr(x0, YuhuAddress(x12, in_bytes(Method::method_data_offset())));
    write_inst_cbz(x0, set_mdp);
    write_insts_final_call_VM_leaf(CAST_FROM_FN_PTR(address, InterpreterRuntime::bcp_to_di), x12, x22);
    // r0: mdi
    // mdo is guaranteed to be non-zero here, we checked for it before the call.
    write_inst_ldr(x1, YuhuAddress(x12, in_bytes(Method::method_data_offset())));
    write_insts_lea(x1, YuhuAddress(x1, in_bytes(MethodData::data_offset())));
    write_inst("add x0, x1, x0");
    write_inst_str(x0, YuhuAddress(x29, frame::interpreter_frame_mdx_offset * wordSize));
    pin_label(set_mdp);
    write_inst_ldp(x0, x1, YuhuAddress(YuhuPost(sp, 2 * wordSize)));
    return current_pc();
}

address YuhuMacroAssembler::write_insts_set_mdp_flag_at(YuhuRegister mdp_in, int flag_byte_constant) {
    assert(ProfileInterpreter, "must be profiling interpreter");
    int header_offset = in_bytes(DataLayout::header_offset());
    int header_bits = DataLayout::flag_mask_to_header_mask(flag_byte_constant);
    // Set the flag
    write_inst_ldr(x8, YuhuAddress(mdp_in, header_offset));
    write_inst("orr %s, %s, #%d", x8, x8, header_bits);
    write_inst_str(x8, YuhuAddress(mdp_in, header_offset));
    return current_pc();
}

address YuhuMacroAssembler::write_insts_update_mdp_by_offset(YuhuRegister mdp_in, int offset_of_disp) {
    assert(ProfileInterpreter, "must be profiling interpreter");
    write_inst_ldr(x8, YuhuAddress(mdp_in, offset_of_disp));
    write_inst_add(mdp_in, mdp_in, x8, lsl, 0);
    write_inst_str(mdp_in, YuhuAddress(x29, frame::interpreter_frame_mdx_offset * wordSize));
    return current_pc();
}

address YuhuMacroAssembler::write_insts_update_mdp_by_offset(YuhuRegister mdp_in, YuhuRegister reg, int offset_of_disp) {
    assert(ProfileInterpreter, "must be profiling interpreter");
    write_insts_lea(x8, YuhuAddress(mdp_in, offset_of_disp));
    write_inst_ldr(x8, YuhuAddress(x8, reg, YuhuAddress::lsl(0)));
    write_inst_add(mdp_in, mdp_in, x8, lsl, 0);
    write_inst_str(mdp_in, YuhuAddress(x29, frame::interpreter_frame_mdx_offset * wordSize));
    return current_pc();
}

address YuhuMacroAssembler::write_insts_update_mdp_for_ret(YuhuRegister return_bci) {
    assert(ProfileInterpreter, "must be profiling interpreter");
    // save/restore across call_VM
    write_inst_stp(xzr, return_bci, YuhuAddress(YuhuPre(sp, -2 * wordSize)));
    write_insts_final_call_VM(noreg,
            CAST_FROM_FN_PTR(address, InterpreterRuntime::update_mdp_for_ret),
            return_bci);
    write_inst_ldp(xzr, return_bci, YuhuAddress(YuhuPost(sp, 2 * wordSize)));
    return current_pc();
}

address YuhuMacroAssembler::write_insts_profile_typecheck(YuhuRegister mdp, YuhuRegister klass, YuhuRegister reg2) {
    if (ProfileInterpreter) {
        YuhuLabel profile_continue;

        // If no method data exists, go to profile_continue.
        write_insts_test_method_data_pointer(mdp, profile_continue);
        ...
        if (TypeProfileCasts) {
            ...
            write_insts_record_klass_in_profile(klass, mdp, reg2, false);
        }
        write_insts_update_mdp_by_constant(mdp, mdp_delta);

        pin_label(profile_continue);
    }
    return current_pc();
}

address YuhuMacroAssembler::write_insts_record_klass_in_profile(YuhuRegister receiver, YuhuRegister mdp, YuhuRegister reg2, bool is_virtual_call) {
    assert(ProfileInterpreter, "must be profiling");
    YuhuLabel done;

    write_insts_record_klass_in_profile_helper(receiver, mdp, reg2, 0, done, is_virtual_call);
...
address YuhuMacroAssembler::write_insts_record_klass_in_profile_helper(YuhuRegister receiver, YuhuRegister mdp,
                                                                       YuhuRegister reg2, int start_row,
                                                                       YuhuLabel& done, bool is_virtual_call) {
    if (TypeProfileWidth == 0) {
        if (is_virtual_call) {
            write_insts_increment_mdp_data_at(mdp, in_bytes(CounterData::count_offset()));
        }
        return current_pc();
    }
    ...
    for (int row = start_row; row <= last_row; row++) {
        ...
        write_insts_test_mdp_data_at(mdp, recvr_offset, receiver,
        ...
        write_insts_increment_mdp_data_at(mdp, count_offset);
        write_inst_b(done);
        pin_label(next_test);

        if (test_for_null_also) {
            YuhuLabel found_null;
            // Failed the equality check on receiver[n]...  Test for null.
            if (start_row == last_row) {
                // The only thing left to do is handle the null case.
                if (is_virtual_call) {
                    write_inst_cbz(reg2, found_null);
                    // Receiver did not match any saved receiver and there is no empty row for it.
                    // Increment total counter to indicate polymorphic case.
                    write_insts_increment_mdp_data_at(mdp, in_bytes(CounterData::count_offset()));
                    write_inst_b(done);
                    pin_label(found_null);
                } else {
                    write_inst_cbnz(reg2, done);
                }
                break;
            }
            ...
            write_insts_record_klass_in_profile_helper(receiver, mdp, reg2, start_row + 1, done, is_virtual_call);
    ...
    write_insts_set_mdp_data_at(mdp, recvr_offset, receiver);
    int count_offset = in_bytes(VirtualCallData::receiver_count_offset(start_row));
    write_insts_mov_imm32(reg2, DataLayout::counter_increment);
    write_insts_set_mdp_data_at(mdp, count_offset, reg2);
    if (start_row > 0) {
        write_inst_b(done);
    }
    return current_pc();
...
address YuhuMacroAssembler::write_insts_profile_typecheck_failed(YuhuRegister mdp) {
    if (ProfileInterpreter && TypeProfileCasts) {
        ...
        write_insts_increment_mdp_data_at(mdp, count_offset, true);
...
address YuhuMacroAssembler::write_insts_profile_null_seen(YuhuRegister mdp) {
    if (ProfileInterpreter) {
        YuhuLabel profile_continue;
        ...
        write_insts_set_mdp_flag_at(mdp, BitData::null_seen_byte_constant());
...
address YuhuMacroAssembler::write_insts_profile_taken_branch(YuhuRegister mdp, YuhuRegister bumped_count) {
    if (ProfileInterpreter) {
        YuhuLabel profile_continue;
        ...
        YuhuAddress data(mdp, in_bytes(JumpData::taken_offset()));
        write_inst_ldr(bumped_count, data);
        assert(DataLayout::counter_increment == 1,
               "flow-free idiom only works with 1");
        ...
        // so we do this
        write_inst("adds %s, %s, #%d", bumped_count, bumped_count, DataLayout::counter_increment);
        YuhuLabel L;
        write_inst_b(cs, L);       // skip store if counter overflow
        write_inst_str(bumped_count, data);
        pin_label(L);
        // The method data pointer needs to be updated to reflect the new target.
        write_insts_update_mdp_by_offset(mdp, in_bytes(JumpData::displacement_offset()));
        pin_label(profile_continue);
    }
    return current_pc();
}

address YuhuMacroAssembler::write_insts_profile_not_taken_branch(YuhuRegister mdp) {
    if (ProfileInterpreter) {
        YuhuLabel profile_continue;
        ...
        write_insts_increment_mdp_data_at(mdp, in_bytes(BranchData::not_taken_offset()));

        // The method data pointer needs to be updated to correspond to
        // the next bytecode
        write_insts_update_mdp_by_constant(mdp, in_bytes(BranchData::branch_data_size()));
        pin_label(profile_continue);
    }
    return current_pc();
}

address YuhuMacroAssembler::write_insts_profile_ret(YuhuRegister return_bci, YuhuRegister mdp) {
    if (ProfileInterpreter) {
        YuhuLabel profile_continue;
        uint row;
        ...
        for (row = 0; row < RetData::row_limit(); row++) {
            ...
            write_insts_test_mdp_data_at(mdp,
                             in_bytes(RetData::bci_offset(row)),
                             return_bci, noreg,
                             next_test);

            // return_bci is equal to bci[n].  Increment the count.
            write_insts_increment_mdp_data_at(mdp, in_bytes(RetData::bci_count_offset(row)));

            // The method data pointer needs to be updated to reflect the new target.
            write_insts_update_mdp_by_offset(mdp,
                                 in_bytes(RetData::bci_displacement_offset(row)));
            write_inst_b(profile_continue);
            pin_label(next_test);
        }

        write_insts_update_mdp_for_ret(return_bci);
...
address YuhuMacroAssembler::write_insts_profile_switch_case(YuhuRegister index, YuhuRegister mdp, YuhuRegister reg2) {
    if (ProfileInterpreter) {
        YuhuLabel profile_continue;
        ...
        write_insts_mov_imm32(w_reg(reg2), in_bytes(MultiBranchData::per_case_size()));
        write_insts_mov_imm32(w8, in_bytes(MultiBranchData::case_array_offset()));
        write_inst_regs("madd %s, %s, %s, %s", w_reg(index), w_reg(index), w_reg(reg2), w8);

        // Update the case count
        write_insts_increment_mdp_data_at(mdp,
                              index,
                              in_bytes(MultiBranchData::relative_count_offset()));

        // The method data pointer needs to be updated.
        write_insts_update_mdp_by_offset(mdp, index, in_bytes(MultiBranchData::relative_displacement_offset()));
...
address YuhuMacroAssembler::write_insts_profile_switch_default(YuhuRegister mdp) {
    if (ProfileInterpreter) {
        YuhuLabel profile_continue;
...
address YuhuMacroAssembler::write_insts_profile_final_call(YuhuRegister mdp) {
    if (ProfileInterpreter) {
        YuhuLabel profile_continue;
        ...
        // The method data pointer needs to be updated to reflect the new target.
        write_insts_update_mdp_by_constant(mdp,
...
YuhuAddress YuhuMacroAssembler::compute_argument_address(YuhuRegisterOrConstant arg_slot, int extra_slot_offset) {
    ...
    if (arg_slot.is_constant()) {
        return YuhuAddress(x20, arg_slot.as_constant() * stackElementSize
                                + offset);
    } else {
        write_inst_add(x8, x20, arg_slot.as_register(), uxtx, exact_log2(stackElementSize));
        return YuhuAddress(x8, offset);
    }
}

address YuhuMacroAssembler::write_insts_profile_obj_type(YuhuRegister obj, const YuhuAddress& mdo_addr) {
    YuhuLabel update, next, none;

    write_insts_verify_oop(obj, atos);

    write_inst_cbnz(obj, update);
    write_insts_orptr(mdo_addr, TypeEntries::null_seen);
    write_inst_b(next);

    pin_label(update);
    write_insts_load_klass(obj, obj);

    write_inst_ldr(x8, mdo_addr);
    write_inst_regs("eor %s, %s, %s", obj, obj, x8);
    write_inst("tst %s, #%d", obj, TypeEntries::type_klass_mask);
    write_inst_b(eq, next); // klass seen before, nothing to
    // do. The unknown bit may have been
    // set already but no need to check.

    write_inst("tst %s, #%d", obj, TypeEntries::type_unknown);
    write_inst_b(ne, next); // already unknown. Nothing to do anymore.

    write_inst_ldr(x8, mdo_addr);
    write_inst_cbz(x8, none);
    write_inst("cmp %s, #%d", x8, TypeEntries::null_seen);
    write_inst_b(eq, none);
    ...
    write_inst_regs("eor %s, %s, %s", obj, obj, x8);
    write_inst("tst %s, #%d", obj, TypeEntries::type_klass_mask);
    write_inst_b(eq, next);

    // different than before. Cannot keep accurate profile.
    write_insts_orptr(mdo_addr, TypeEntries::type_unknown);
    write_inst_b(next);

    pin_label(none);
    // first time here. Set profile type.
    write_inst_str(obj, mdo_addr);

    pin_label(next);
    return current_pc();
}

address YuhuMacroAssembler::write_insts_profile_arguments_type(YuhuRegister mdp, YuhuRegister callee, YuhuRegister tmp, bool is_virtual) {
    if (!ProfileInterpreter) {
        return current_pc();
    }
    ...
    if (MethodData::profile_arguments() || MethodData::profile_return()) {
        ...
        write_inst_ldrb(w8, YuhuAddress(mdp, in_bytes(DataLayout::tag_offset()) - off_to_start));
        write_inst("cmp %s, #%d", x8, is_virtual ? DataLayout::virtual_call_type_data_tag : DataLayout::call_type_data_tag);
        write_inst_b(ne, profile_continue);
        ...
        if (MethodData::profile_arguments()) {
            ...
            for (int i = 0; i < TypeProfileArgsLimit; i++) {
                if (i > 0 || MethodData::profile_return()) {
                    ...
                    write_inst_ldr(tmp, YuhuAddress(mdp, in_bytes(TypeEntriesAtCall::cell_count_offset())));
                    write_inst("sub %s, %s, #%d", tmp, tmp, i*TypeStackSlotEntries::per_arg_count());
                    write_inst("cmp %s, #%d", tmp, TypeStackSlotEntries::per_arg_count());
                    write_inst("add %s, %s, #%d", x8, mdp, off_to_args);
                    write_inst_b(lt, done);
                }
                write_inst_ldr(tmp, YuhuAddress(callee, Method::const_offset()));
                write_insts_load_unsigned_short(w_reg(tmp), YuhuAddress(tmp, ConstMethod::size_of_parameters_offset()));
                // stack offset o (zero based) from the start of the argument
                // list, for n arguments translates into offset n - o - 1 from
                // the end of the argument list
                write_inst_ldr(x8, YuhuAddress(mdp, in_bytes(TypeEntriesAtCall::stack_slot_offset(i))));
                write_inst_regs("sub %s, %s, %s", tmp, tmp, x8);
                write_inst("sub %s, %s, #%d", tmp, tmp, 1);
                YuhuAddress arg_addr = compute_argument_address(tmp);
                write_inst_ldr(tmp, arg_addr);

                YuhuAddress mdo_arg_addr(mdp, in_bytes(TypeEntriesAtCall::argument_type_offset(i)));
                write_insts_profile_obj_type(tmp, mdo_arg_addr);
            ...
            if (MethodData::profile_return()) {
                write_inst_ldr(tmp, YuhuAddress(mdp, in_bytes(TypeEntriesAtCall::cell_count_offset())));
                write_inst("sub %s, %s, #%d", tmp, tmp, TypeProfileArgsLimit*TypeStackSlotEntries::per_arg_count());
            }

            write_inst("add %s, %s, #%d", x8, mdp, off_to_args);
            pin_label(done);
            write_inst_mov_reg(mdp, x8);
            ...
            if (MethodData::profile_return()) {
                ...
                write_inst_add(mdp, mdp, tmp, lsl, exact_log2(DataLayout::cell_size));
            }
            write_inst_str(mdp, YuhuAddress(x29, frame::interpreter_frame_mdx_offset * wordSize));
        } else {
...
address YuhuMacroAssembler::write_insts_profile_virtual_call(YuhuRegister receiver, YuhuRegister mdp, YuhuRegister reg2, bool receiver_can_be_null) {
    if (ProfileInterpreter) {
        YuhuLabel profile_continue;
        ...
        YuhuLabel skip_receiver_profile;
        if (receiver_can_be_null) {
            YuhuLabel not_null;
            // We are making a call.  Increment the count for null receiver.
            write_insts_increment_mdp_data_at(mdp, in_bytes(CounterData::count_offset()));
            write_inst_b(skip_receiver_profile);
            pin_label(not_null);
        }

        // Record the receiver type.
        write_insts_record_klass_in_profile(receiver, mdp, reg2, true);
        pin_label(skip_receiver_profile);
...
address YuhuMacroAssembler::write_insts_profile_call(YuhuRegister mdp) {
    if (ProfileInterpreter) {
        YuhuLabel profile_continue;
        ...
        // The method data pointer needs to be updated to reflect the new target.
        write_insts_update_mdp_by_constant(mdp, in_bytes(CounterData::counter_data_size()));
        pin_label(profile_continue);
    }
    return current_pc();
}
